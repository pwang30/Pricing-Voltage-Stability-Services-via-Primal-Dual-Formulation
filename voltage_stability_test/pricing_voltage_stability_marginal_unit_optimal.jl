...
#Pˢᴳₘₐₓ= [60, 55, 50, 45, 40, 35]*1.2       # ACtive max generation of SGs,  buses:2,3,4,5,27,30
...
#Kˢᵗ=[200, 125, 92.5, 72, 55, 31]              #  Startup cost of SGs                     SGs, buses:2,3,4,5,27,30
#Kˢʰ=[50, 28.5, 18.5, 14.4, 12, 10]            #  Shutdown cost of SGs                    SGs, buses:2,3,4,5,27,30
#Oᵐ=[6.20, 7.10, 10.47, 12.28, 13.53, 15.36]                     #  Marginal generation cost of SGs   in    SGs, buses:2,3,4,5,27,30
#Oⁿˡ=[20.431, 15.005, 13.755, 10.930, 9.900, 8.570]*15*5              #  No-load cost of SGs                     SGs, buses:2,3,4,5,27,30
...
#Oⁿˡ=[15.005, 15.005, 15.005, 15.005, 15.005, 15.005]*10 
Oⁿˡ=Oᵐ*10 
...
#---------------------------------------------------------------------------------------------------------------------------
#  AC power flow constraints with SOCP relaxation, refer this paper for math equations:  https://arxiv.org/pdf/1504.06770
#---------------------------------------------------------------------------------------------------------------------------
...
@variable(model, obj[1:T])


for t in 1:T
    @constraint(model, cost_onoff_Primal[t] == Cᵁ²[t] + Cᴰ²[t] + Cᵁ³[t] + Cᴰ³[t] + Cᵁ⁴[t] + Cᴰ⁴[t] + Cᵁ⁵[t] + Cᴰ⁵[t] + Cᵁ²⁷[t] + Cᴰ²⁷[t] + Cᵁ³⁰[t] + Cᴰ³⁰[t])
    @constraint(model, cost_nl_Primal[t] == Oⁿˡ[1].*(yˢᴳ²[t]) + Oⁿˡ[2].*(yˢᴳ³[t]) + Oⁿˡ[3].*(yˢᴳ⁴[t]) + Oⁿˡ[4].*(yˢᴳ⁵[t]) + Oⁿˡ[5].*(yˢᴳ²⁷[t]) + Oⁿˡ[6].*(yˢᴳ³⁰[t]) )
    @constraint(model, cost_gene_Primal[t] == Oᵐ[1].*Pᴳ[2,t] + Oᵐ[2].*Pᴳ[3,t] + Oᵐ[3].*Pᴳ[4,t] + Oᵐ[4].*Pᴳ[5,t] + Oᵐ[5].*Pᴳ[27,t] + Oᵐ[6].*Pᴳ[30,t] )
    @constraint(model, obj[t] == cost_onoff_Primal[t] + cost_nl_Primal[t] + cost_gene_Primal[t] )
end


@objective(model, Min, sum(obj))  # single-level objective function
...
# Analyze results
#------------------------------
obj_original_value=JuMP.value.(obj)

plot(obj_original_value)