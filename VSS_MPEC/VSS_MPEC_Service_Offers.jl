# Calculate service offers for generators using restricted pricing method
function service_offer(K_c_gc, K_c_gv, K_c_m, sh_1, sh_2, bus_num, T, Pᴰ, Qᴰ, bus_connection, S_lines, 
...
#----------------------------------------------------------------
#  UC model for optimal commitments of SGs
#----------------------------------------------------------------
...
for t in 1:T
    ...
    @constraint(model, [Q_23_eq[t] + Γ_23[t]*S_base, P_23_eq[t], Q_23_eq[t]] in SecondOrderCone()  )      
    @constraint(model, [Q_24_eq[t] + Γ_24[t]*S_base, P_24_eq[t], Q_24_eq[t]] in SecondOrderCone()  )
...
#--------------------------Recording optimal commitments of SGs--------------------------
...
#----------------------------------------------------------------
#  UC model for calculating the honest service offers
#----------------------------------------------------------------
...
for t in 1:T
    @constraint(model, yˢᴳ²[t] ==yˢᴳ²_opt[t])      # assign the optimal UC schedule to the decision variables
    @constraint(model, yˢᴳ³[t] ==yˢᴳ³_opt[t])
    @constraint(model, yˢᴳ⁴[t] ==yˢᴳ⁴_opt[t])
    @constraint(model, yˢᴳ⁵[t] ==yˢᴳ⁵_opt[t])
    @constraint(model, yˢᴳ²⁷[t] ==yˢᴳ²⁷_opt[t])
    @constraint(model, yˢᴳ³⁰[t] ==yˢᴳ³⁰_opt[t])
...
for t in 1:T
    ...
    Voltage_constraint_23[t] = @constraint(model, [Q_23_eq[t] + Γ_23[t]*S_base, P_23_eq[t], Q_23_eq[t]] in SecondOrderCone()  )      
    Voltage_constraint_24[t] = @constraint(model, [Q_24_eq[t] + Γ_24[t]*S_base, P_24_eq[t], Q_24_eq[t]] in SecondOrderCone()  )
...
#--------------------------Dualize and obtain honest service offers--------------------------
...
price_Γ_23=mu[:,1]
price_Γ_24=mu[:,2]
price_P_23=-lambda_1[:,1]
price_P_24=-lambda_1[:,2]
price_Q_23=-lambda_2[:,1]+mu[:,1]
price_Q_24=-lambda_2[:,2]+mu[:,2]


return price_Γ_23, price_Γ_24, price_P_23, price_P_24, price_Q_23, price_Q_24
