...
#-----------------------------------Dataset Classification
    K_c_gc=zeros(4,a)   # define the linearized coffecoents for SGs
    K_c_gv=zeros(4,1)    # define the linearized coffecoents for VSGs
    K_c_m=zeros(4,a+b)   # define the linearized coffecoents for interactions of each two (V)SGs.   78=66+12
...
#-----------------------------------Define Offline-Training Model.  z₂₃¹
        ...
        for i in 1:size(zᵟ_c,1)
            ...
            @constraint(Training_z₂₃¹, penalty_z₂₃¹[i]==z_23_1[i] -zᵟ_c[i,3])
        end
...
#-----------------------------------Define Offline-Training Model.  z₂₄¹
        ...
        for i in 1:size(zᵟ_c,1)
            ...
            @constraint(Training_z₂₄¹, penalty_z₂₄¹[i]==z_24_1[i] -zᵟ_c[i,4])
        end
...
K_23_gc = value.(K_23_gc)
K_24_gc = value.(K_24_gc)
K_23_gv = value.(K_23_gv)
K_24_gv = value.(K_24_gv)
K_23_m = value.(K_23_m)
K_24_m = value.(K_24_m)
...
K_c_m[4,:]=K_24_1_m
    ...
    return  K_c_gc, K_c_gv, K_c_m, sh_1, sh_2 #shape_control
end
